...
<body>
				...
				<div class="span6" style="margin-left: 300px;">
					<div class="project-description">
						<div class="project-title clearfix">
							<h3>${movieS.moTitle}</h3>

						</div>
						<div class="project-info">

							<div>
								<span>상영시간</span>${movieS.moTime}</div>
							<div>
								<span>장&nbsp;&nbsp;&nbsp;&nbsp;르</span>${movieS.moGenre}</div>
							<div>
								<span>개봉일</span>${movieS.moDate}</div>
							<div>
								<span>출연진</span>${movieS.moAct}</div>
						
								<span>줄거리</span>
								<p>${movieS.moContents}</p>
						</div>
					...
					<c:choose>
						...
						<c:when test="${sessionScope.loginId != null}">
							...
							<form action="MovieSaveDelete" method="POST" name="movieDelete"
								...
								<input type="button" onclick="location.href='reviewList?moNum=${movieS.moNum}'"
	...
	<div class="section secondary-section">
		<div class="container centered">
			<a href="MovieList.jsp" class="button">목록보기</a> <input type="button"
				value="뒤로가기" onclick="history.back(-1);" class="button">
		</div>
	</div>
